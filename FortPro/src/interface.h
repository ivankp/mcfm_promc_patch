...
void promcinit(int myid, char *workdir, char *filename, int maxIter,
               int nrCalls, double Energy);
void promcclose();
...
extern "C" {
  ...
  void FortranCInterface_GLOBAL(promcinit, PROMCINIT) (
    int *myid, char *workdir, char *filename, int *maxIter,
    int *nrCalls, double *Energy)
  {
  ...
  void FortranCInterface_GLOBAL(promcaddparticle, PROMCADDPARTICLE) (
    int *barcode, double *px, double *py, double *pz, double *e)
  {
  ...
  void FortranCInterface_GLOBAL(promcaddstats, PROMCADDSTATS) (
    double *cross, double *crosserr, double *lumi, int *events)
  {
